...
#ifndef _GENERIC_CHAR_H
#define _GENERIC_CHAR_H
 ...
 *
 * - Generating the Major number dynamically.
 * - Using class_create() and device_create() to create the device node.
 *
 * Thanks to class_create() and device_create(), it is not necessary to use
 * the command mknod.
 */
static inline int __init my_generic_init(void)
{
	...
	my_dev = device_create(foo_class, NULL, first, NULL, "%s", MYDEV_NAME);
 ...
 * MODULE_DESCRIPTION("LFD5435:5.4 s_13/lab_char.h");
