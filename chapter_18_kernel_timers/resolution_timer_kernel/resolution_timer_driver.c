 ...
 *
 * sudo insmod resolution_timer_driver.ko
 * dmesg
 * more /dev/resolution_timer
 * chmod 666 /dev/resolution_timer
 * sudo chmod 666 /dev/resolution_timer
 * more /dev/resolution_timer
 * echo 1 > /dev/resolution_timer
 * more /dev/resolution_timer
 * sudo rmmod resolution_timer_driver
 */
	     ...
	     loff_t *ppos)
{
	...
	tl= &data->timer;
	tl->expires = jiffies + HZ;	/* one second delay */
...
static const struct file_operations resolution_timer_fops = {
	.owner = THIS_MODULE,
...
{
	/*
	   delete any running timers. The timer_initd variable avoids
