/* **************** LFD5435:5.4 s_19/lab_ioctl_signal.c **************** */
/*
...
mycdrv_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case MYIOC_SETSIG:
		sig_tosend = (int)arg;
		dev_info(my_dev, " Setting signal to send as: %d\n",
			 sig_tosend);
		break;
	case MYIOC_SENDSIG:
		if (!sig_tsk) {
		...
		dev_info(my_dev, " Sending signal %d to process ID %d\n",
			 sig_tosend, sig_pid);
		retval = send_sig(sig_tosend, sig_tsk, 0);
		dev_info(my_dev, "retval = %d\n", retval);
...
MODULE_DESCRIPTION("LFD5435:5.4 s_19/lab_ioctl_signal.c");
