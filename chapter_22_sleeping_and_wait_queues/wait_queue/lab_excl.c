 ...
 */
/*
 *  Using Wait Queues  (exclusive wait solution)
 *
...
{
	dev_info(my_dev, "read() Process %i (%s) going to sleep\n", current->pid,
		 current->comm);
	atomic_inc(&nsleepers);
	wait_event_interruptible_exclusive(wq, (atomic_read(&data_ready)));
	dev_info(my_dev, "read() process %i (%s) awakening\n", current->pid,
		 current->comm);
	     ...
	     loff_t *ppos)
{
	...
	dev_info(my_dev, "write() process %i (%s) awakening the readers...\n",
...
MODULE_DESCRIPTION("LFD5435:5.4 s_22/lab_excl.c");
