 ...
 *
 * Take the wait_event_interruptible() solution and extend it to have
 * a poll() entry point.
 *
...
static unsigned int mycdrv_poll(struct file *file, poll_table *wait)
{
	...
	dev_info(my_dev, "In poll at jiffies=%ld\n", jiffies);
...
MODULE_DESCRIPTION("LFD5435:5.4 s_22/lab_poll.c");
